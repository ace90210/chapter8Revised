...
public class MathGame extends GLGame {
	public Screen getStartScreen()
	{
		return new MathGameScreen(this);
	}
	
	class MathGameScreen extends Screen implements OnKeyListener {
		Camera2D camera;
		final float WORLD_WIDTH = 820.0f;
		int BALL_LIMIT = 20;
		int SPRITE_LIMIT = 100;
		int spawned = 0;
		...
		public MathGameScreen(Game game) {
				super(game);
				Random rand = new Random();
				BALL_LIMIT = rand.nextInt(20) + 10;
				SPRITE_LIMIT = BALL_LIMIT * 2 + (rand.nextInt(70));				
		...
		public void present(float deltaTime) {
			...
			if(len != SPRITE_LIMIT) {
				if(spawned == BALL_LIMIT && len > SPRITE_LIMIT / 2) {
					glText.begin( 1.0f, 0.0f, 0.0f, 1.0f );         // Begin Text Rendering (Set Color WHITE)
					glText.setScale(camera.zoom * 3);
			        glText.drawC( "You Lost!!!", camera.position.x, camera.position.y);              // Draw Test String
			        glText.end(); 
				}
				else {
					glText.begin( 1.0f, 1.0f, 1.0f, 1.0f );         // Begin Text Rendering (Set Color WHITE)
					glText.setScale(camera.zoom);
			        glText.drawC( "Target = " + SPRITE_LIMIT + "   Spawn Limit: " + BALL_LIMIT + "   Spawned: " + spawned, camera.position.x, camera.position.y + (camera.frustumHeight / 2) * camera.zoom - (20 * camera.zoom) );              // Draw Test String
			        glText.drawC( "Current: " + len, camera.position.x, camera.position.y + (camera.frustumHeight / 2) * camera.zoom - (20 * camera.zoom) - 25);              // Draw Test String
				...
				glText.setScale(camera.zoom * 3);
		        glText.drawC( "Congratulations You Won!!!", camera.position.x, camera.position.y);              // Draw Test String
		        glText.end();                                   // End Text Rendering  
			}
		}
