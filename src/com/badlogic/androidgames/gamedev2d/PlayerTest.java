...
public class PlayerTest extends GLGame {
	public Screen getStartScreen()
	{
		return new PlayerScreen(this);
	}
	
	class PlayerScreen extends Screen implements OnKeyListener {
		Camera2D camera;
		...
		public void update(float deltaTime) {
			...
			if(!paused) {
				...
				//move player
				if(isKeyPressed(KeyEvent.KEYCODE_DPAD_RIGHT) && player.position.x + player.bounds.width / 2 < WORLD_WIDTH) 	{
					player.velocity.x = 250;
					playerState = AnimationState.WALKING_RIGHT;
				} else if(isKeyPressed(KeyEvent.KEYCODE_DPAD_LEFT) && player.position.x - player.bounds.width / 2 > 0) 	{
					player.velocity.x = -250;
		...
		public void present(float deltaTime) {
			...
			if(balls.size() > 0) {
				batcher.beginBatch(ballTexture);
				for(int i = 0; i < balls.size(); i++) {
					DynamicGameObject ball = balls.get(i);
			...
			if(!alive) {
				...
				glText.drawC( "Press select to try again", camera.position.x, camera.position.y - 30);              // Draw Test String
				glText.end(); 
			...
			glText.begin( 1.0f, 1.0f, 1.0f, 1.0f );         // Begin Text Rendering (Set Color WHITE)
			glText.setScale(camera.zoom);
	        glText.drawC( "Spear Break Y: " + (int)spear.position.y + "   Spear Speed: " + SPEAR_SPEED, camera.position.x, camera.position.y + (camera.frustumHeight / 2) * camera.zoom - (20 * camera.zoom) );              // Draw Test String
		...
		public void resume() {
			...
			ballTexture = new Texture(((GLGame)game), "ballPang.png");
			ballRegion = new TextureRegion(ballTexture, 0, 0, 256, 256);
		}
